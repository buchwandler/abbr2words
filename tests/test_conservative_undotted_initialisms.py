...
@pytest.mark.parametrize("token", ["NGO", "SSP", "PTM", "HPRD", "ACM", "GPS", "FBI"])
...
def test_conservative_mode_keeps_registered_precedence_and_speech_policy() -> None:
    assert abbr2words("BBC NGO", initialism_mode="conservative_undotted") == "B B C N G O"
...
def test_protected_spans_and_url_like_text_are_absolute() -> None:
    source = "See NGO at https://ABC.example"
    ...
    assert (
        ...
        == "See N G O at https://ABC.example"
    )
...
def test_conservative_replacements_are_source_aligned_and_named() -> None:
    ...
    assert result.text == "n g o f b i"
    assert [
        (item.start, item.end, source[item.start : item.end], item.source, item.rule)
        for item in result.replacements
    ] == [
        (0, 3, "NGO", "abbr:initialism-conservative", "abbr:initialism-conservative"),
        (4, 7, "FBI", "abbr:initialism-conservative", "abbr:initialism-conservative"),
    ]
...
def test_diagnostics_expose_reason_codes_and_registered_identity() -> None:
    source = "BBC NGO WORLD FIRST ISO-9001 XIX NASA IN E.D."
    rows = list(
        iter_initialism_diagnostics(
            source,
            initialism_mode="conservative_undotted",
    ...
    assert by_source["NGO"].reason == "headline-run"
    assert by_source["ISO-9001"].reason == "alphanumeric-identifier"
    ...
    assert by_source["E.D."].reason == "dotted-fallback"
...
def test_diagnostics_report_protection_and_structured_candidates() -> None:
    source = "NGO AAPL https://ABC.example"
...
def test_unicode_neighbors_do_not_extend_or_throw() -> None:
    ...
    assert abbr2words("Ω NGO Ω", initialism_mode="conservative_undotted") == "Ω N G O Ω"
...
def test_conservative_acceptance_is_a_subset_of_broad_acceptance() -> None:
    samples = [
        ...
        for chars in product("ABCFGNOPSTVX", repeat=length)
    ][:400]
