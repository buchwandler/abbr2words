...
| Arabic               | `ar`                                                                                            | Unicode CLDR Arabic units/eras; KSU title/reference guidance; Saudi institutional date usage | CLDR + BIPM SI  | source-backed conservative baseline; native review pending      |
| RTL                  | `fa`, `he`                                                                                      | national orthography guidance                                                                | BIPM SI + CLDR  | conservative agent seed; native review pending                  |
...
- **Unicode CLDR 48 Arabic unit grammar and locale summary**. Neutral unit labels, Arabic calendar-era markers (`م`, `هـ`, `ق.م`), and Arabic numbering evidence. https://www.unicode.org/cldr/charts/48/grammar/ar.html and https://www.unicode.org/cldr/charts/48/summary/ar.html
- **King Saud University official correspondence guide**. Professional title abbreviations `د.` and `م.`. https://faculty.ksu.edu.sa/sites/default/files/190710_dlyl_lmrslt.pdf
- **Saudi Capital Market Authority and Saudi Press Agency usage**. Spaced and attached Gregorian/Hijri date markers. https://cma.org.sa/Market/Circulars/Documents/SAAB_Shareholder_Circular.pdf and https://www.spa.gov.sa/ar/
- **King Saud University reference usage**. Numeric page-marker forms `ص.` and `ص`. https://engineering.ksu.edu.sa/ar/CE_faculty
- **BIPM International System of Units**. International input symbols and capitalization retained by the shared parser. https://www.bipm.org/en/measurement-units/si-base-units

Implementation policy:

- Arabic uses a source-backed conservative baseline. All 38 shared common-unit identities have neutral Arabic labels, while numeric unit context remains responsible for preventing standalone-symbol expansions.
- `م.` expands to `مهندس` only before a lexical name and not after numeric text. Numeric Gregorian `م`, Hijri `هـ`, and historical `ق.م` require a numeric-year context. Dotless `ص` requires a following number.
- Unit output does not perform Arabic number agreement, case, gender, dialect selection, or number-to-words conversion.
- Generic currencies, region-specific academic ranks, ambiguous Arabic-letter unit aliases, and optional bibliographic abbreviations remain out of scope.

The checked-in Arabic source IDs are `ar-unicode-cldr-48-units`, `ar-unicode-cldr-48-calendar-eras`, `ar-ksu-official-correspondence-titles`, `ar-saudi-official-era-usage`, `ar-ksu-reference-style`, and `bipm-si`. Native-speaker review is still recommended before promotion beyond the baseline tier.
...
| ----------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------- |
| `am az be bn ca ce cy da eo fa fi he hi hu hy id is kn kz lt lv mn no ro sk sl sr te tet tg uk` | `language-style-baseline`, `unicode-cldr-48.2.1`                                                                                                                                 | guarded reference/title baseline and neutral units                                             | linguistically-reviewed pending native review                   |
| `ar`                                                                                            | `ar-unicode-cldr-48-units`, `ar-unicode-cldr-48-calendar-eras`, `ar-ksu-official-correspondence-titles`, `ar-saudi-official-era-usage`, `ar-ksu-reference-style`, `bipm-si`      | guarded title/reference/calendar rules; complete localized neutral units                       | source-backed conservative baseline; native review pending      |
